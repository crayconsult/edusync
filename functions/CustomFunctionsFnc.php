...
function Prompt_Copy_School($title = 'Confirm', $question = '', $message = '', $pdf = '') {
    ...
    if (!$_REQUEST['delete_ok'] && !$_REQUEST['delete_cancel']) {
        echo '<BR>';
        PopTable('header', $title);
        echo "<h2 class=\"no-margin-top\">$question</h2><FORM class=no-margin name=prompt_form id=prompt_form action=$PHP_tmp_SELF&delete_ok=1 METHOD=POST>$message<hr class=\"no-margin-top\"><INPUT type=submit class=\"btn btn-primary\" value=OK onclick='formcheck_school_setup_copyschool();'>&nbsp;<INPUT type=button class=\"btn btn-primary\" name=delete_cancel value=Cancel onclick='load_link(\"Modules.php?modname=schoolsetup/Calendar.php\");'></FORM>";
        PopTable('footer');
        return false;
    } else
        return true;
